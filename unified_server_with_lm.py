...
DEFAULT_CHECKPOINT = r"C:\YonAPIAPI\models\API_ready_model\best_model.pt"
DEFAULT_DEPLOYMENT = r"C:\YonAPIAPI\deployment_data"

# Language Model Configuration
LM_MODEL_NAME = "rasyosef/roberta-base-amharic"  # Best Amharic LM
USE_LANGUAGE_MODEL = True  # Set to False to disable LM
...
class AmharicLanguageModel:
    ...
    def _tokenize_and_mask(self, text: str, mask_token: str = '<mask>') -> List[str]:
        ...
        # Simple approach: split by spaces and mask each word
        words = text.split()
        candidates = []
        for i, word in enumerate(words):
    ...
    def correct_text(self, text: str, confidence: float = 0.8) -> Dict[str, Any]:
        ...
        try:
            # Use fill-mask pipeline to suggest corrections
            # We'll process each word separately
            words = text.split()
            if len(words) < 1:
                return {'original': text, 'corrected': text, 'corrections': [], 'was_corrected': False, 'confidence': confidence}
            
            # For each word, mask it and get predictions
            for i, word in enumerate(words):
                ...
                # Find the best prediction that is different from original
...
async def health_check():
    return {
        ...
        "device": DEVICE,
        "max_strokes": MAX_STROKES,
        "lm_available": LM_AVAILABLE and inference_engine and inference_engine.lm and inference_engine.lm.available if inference_engine else False
    }
...
async def model_info():
    ...
    return {
        ...
        "memory_size": MEMORY_SIZE,
        "device": DEVICE,
        "lm_available": inference_engine.lm is not None and inference_engine.lm.available if inference_engine else False,
    ...
    try:
        async for message in websocket:
            ...
            if command == "load_model":
                ...
                enable_lm = data.get("enable_lm", USE_LANGUAGE_MODEL)
                ...
                try:
                    ...
                    await websocket.send(json.dumps({
                        ...
                        "lm_available": websocket_engine.lm is not None and websocket_engine.lm.available
                    }))
